...
int main(int ac, char **av) {
    ...
    // Create the ULR view.
    GaussianView::Ptr gaussianView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile, &messageRead,
                                                    white_background, !myArgs.noInterop, device, myArgs.appearance_id,
    ...
    generalCamera->setup(scene->cameras()->inputCameras(),
                         Viewport(0, 0, (float)usedResolution.x(), (float)usedResolution.y()), nullptr);
    ...
    multiViewManager.addIBRSubView("Point view", gaussianView, usedResolution,
    ...
    if (myArgs.pathFile.get() != "") {
        ...
        generalCamera->getCameraRecorder().recordOfflinePath(myArgs.outPath,
