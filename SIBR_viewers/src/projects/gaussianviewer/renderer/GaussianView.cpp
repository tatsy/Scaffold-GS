...
#define CUDA_SAFE_CALL_ALWAYS(A) \
  ...
  if (cudaPeekAtLastError() != cudaSuccess) SIBR_ERR << cudaGetErrorString(cudaGetLastError());
...
int loadPly(const char *filename, std::vector<float> &pos, std::vector<float> &normal, std::vector<float> &offset,
            std::vector<float> &feat, std::vector<float> &opacity, std::vector<float> &scale1,
            std::vector<float> &scale2, std::vector<float> &rot, std::vector<float> &gs_pos, sibr::Vector3f &minn,
            sibr::Vector3f &maxx) {
    std::ifstream infile(filename, std::ios_base::binary);

    if (!infile.good()) SIBR_ERR << "Unable to find model's PLY file, attempted:\n" << filename << std::endl;
...
void savePly(const char *filename, std::vector<float> &pos, std::vector<float> &normal, std::vector<float> &offset,
             std::vector<float> &feat, std::vector<float> &opacity, std::vector<float> &scale1,
             std::vector<float> &scale2, std::vector<float> &rot, const sibr::Vector3f &minn,
             const sibr::Vector3f &maxx) {
    // Read all Gaussians at once (AoS)
    int count = 0;
    for (int i = 0; i < pos.size() / 3; i++) {
        if (pos[3 * i] < minn.x() || pos[3 * i + 1] < minn.y() || pos[3 * i + 2] < minn.z() || pos[3 * i] > maxx.x()
            || pos[3 * i + 1] > maxx.y() || pos[3 * i + 2] > maxx.z())
            continue;
        count++;
    }
    std::vector<AnchorPoint> points(count);
    ...
    std::string props_offset[] = { "f_offset_0",  "f_offset_1",  "f_offset_2",  "f_offset_3",  "f_offset_4",
                                   "f_offset_5",  "f_offset_6",  "f_offset_7",  "f_offset_8",  "f_offset_9",
                                   "f_offset_10", "f_offset_11", "f_offset_12", "f_offset_13", "f_offset_14",
                                   "f_offset_15", "f_offset_16", "f_offset_17", "f_offset_18", "f_offset_19",
                                   "f_offset_20", "f_offset_21", "f_offset_22", "f_offset_23", "f_offset_24",
                                   "f_offset_25", "f_offset_26", "f_offset_27", "f_offset_28", "f_offset_29" };
    std::string props_feat[] = { "f_anchor_feat_0",  "f_anchor_feat_1",  "f_anchor_feat_2",  "f_anchor_feat_3",
                                 "f_anchor_feat_4",  "f_anchor_feat_5",  "f_anchor_feat_6",  "f_anchor_feat_7",
                                 "f_anchor_feat_8",  "f_anchor_feat_9",  "f_anchor_feat_10", "f_anchor_feat_11",
                                 "f_anchor_feat_12", "f_anchor_feat_13", "f_anchor_feat_14", "f_anchor_feat_15",
                                 "f_anchor_feat_16", "f_anchor_feat_17", "f_anchor_feat_18", "f_anchor_feat_19",
                                 "f_anchor_feat_20", "f_anchor_feat_21", "f_anchor_feat_22", "f_anchor_feat_23",
                                 "f_anchor_feat_24", "f_anchor_feat_25", "f_anchor_feat_26", "f_anchor_feat_27",
                                 "f_anchor_feat_28", "f_anchor_feat_29", "f_anchor_feat_30", "f_anchor_feat_31" };
    ...
    for (auto s : prop_pos) outfile << "property float " << s << std::endl;
    for (auto s : prop_normal) outfile << "property float " << s << std::endl;
    for (auto s : props_offset) outfile << "property float " << s << std::endl;
    for (auto s : props_feat) outfile << "property float " << s << std::endl;
    for (auto s : props_opacity) outfile << "property float " << s << std::endl;
    for (auto s : props_base_offset) outfile << "property float " << s << std::endl;
    for (auto s : props_base_scale) outfile << "property float " << s << std::endl;
...
sibr::GaussianView::GaussianView(const sibr::BasicIBRScene::Ptr &ibrScene, uint render_w, uint render_h,
                                 std::string plyPath, bool *messageRead, bool white_bg, bool useInterop, int device,
    ...
    count = loadPly(ply_path.c_str(), anchor_pos, anchor_normal, anchor_offset, anchor_feature, anchor_opacity,
                    anchor_scale_1, anchor_scale_2, anchor_rotation, gaussian_pos, _scenemax, _scenemin);
    ...
    ak_rot_all =
        torch::nn::functional::normalize(ak_rot_all, torch::nn::functional::NormalizeFuncOptions().p(2).dim(-1));
    ...
    if (currMode == "Initial Points") {
        ...
        if (count != 0) {
            ...
            CudaRasterizer::Rasterizer::visible_filter(
                geomBufferFunc_filter, binningBufferFunc_filter, imgBufferFunc_filter, count, M, W, H,
                ak_pos_all.contiguous().data_ptr<float>(), ak_scale_1.contiguous().data_ptr<float>(), scale_modifier,
                ak_rot_all.contiguous().data_ptr<float>(), nullptr, viewmatrix.contiguous().data_ptr<float>(),
                projmatrix.contiguous().data_ptr<float>(), tan_fovx, tan_fovy, 0, radii.contiguous().data_ptr<int>());
        }
        ...
        torch::Tensor all =
            torch::cat({ gs_pos, gs_scale, gs_color, scale_rot, neural_opacity }, -1).to(_libtorch_device);
        ;
        ...
        torch::Tensor gs_rot = torch::nn::functional::normalize(
            all.narrow(-1, 12, 4), torch::nn::functional::NormalizeFuncOptions().p(2).dim(-1));
        torch::Tensor gs_opacity = all.narrow(-1, 16, 1);

        // Copy frame-dependent data to GPU
        Eigen::Vector3f pos = eye.position();
        CUDA_SAFE_CALL(cudaMemcpy(cam_pos_cuda, &pos, sizeof(float) * 3, cudaMemcpyHostToDevice));
        ...
        CudaRasterizer::Rasterizer::forward(
            geomBufferFunc, binningBufferFunc, imgBufferFunc, P, 1, 16, background_cuda, _resolution.x(),
            _resolution.y(), gs_pos.contiguous().data_ptr<float>(), nullptr, gs_color.contiguous().data_ptr<float>(),
            gs_opacity.contiguous().data_ptr<float>(), gs_scale.contiguous().data_ptr<float>(), _scalingModifier,
            gs_rot.contiguous().data_ptr<float>(), nullptr, view_cuda, proj_cuda, cam_pos_cuda, tan_fovx, tan_fovy,
            false, image_cuda, nullptr, rects, boxmin, boxmax);
...
void sibr::GaussianView::onGUI() {
    ...
    if (_cropping) {
        ...
        if (ImGui::Button("Save")) {
            savePly(_buff, anchor_pos, anchor_normal, anchor_offset, anchor_feature, anchor_opacity, anchor_scale_1,
                    anchor_scale_2, anchor_rotation, _boxmin, _boxmax);
        }
    }

    ImGui::End();

    if (!*_dontshow && !accepted && _interop_failed) ImGui::OpenPopup("Error Using Interop");
