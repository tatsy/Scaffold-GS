...
static const char *jResX = "resolution_x";
static const char *jResY = "resolution_y";
static const char *jFovY = "fov_y";
static const char *jFovX = "fov_x";
static const char *jZFar = "z_far";
static const char *jZNear = "z_near";
static const char *jTrain = "train";
static const char *jViewMat = "view_matrix";
static const char *jViewProjMat = "view_projection_matrix";
static const char *jScalingModifier = "scaling_modifier";
static const char *jSHsPython = "shs_python";
static const char *jRotScalePython = "rot_scale_python";
static const char *jKeepAlive = "keep_alive";
            ...
            while (keep_running) {
                {
                    ...
                    sendData[jViewMat] =
                        std::vector<float>((float *)&_remoteInfo.view, ((float *)&_remoteInfo.view) + 16);
                    sendData[jViewProjMat] =
                        std::vector<float>((float *)&_remoteInfo.viewProj, ((float *)&_remoteInfo.viewProj) + 16);
    ...
    if (_showSfM || _timestampReceived == 0 || (preview && _renderSfMInMotion)) {
    ...
    } else {
        {
            ...
            if (_imageResize) {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, _resolution.x(), _resolution.y(), 0, GL_RGB, GL_UNSIGNED_BYTE,
                             0);
                glBindTexture(GL_TEXTURE_2D, 0);
                _imageResize = false;
            }
            if (_imageDirty && _imageData.size() == 3 * _resolution.x() * _resolution.y()) {
                glTextureSubImage2D(_imageTexture, 0, 0, 0, _resolution.x(), _resolution.y(), GL_RGB, GL_UNSIGNED_BYTE,
                                    _imageData.data());
                _imageDirty = false;
            }
        }
