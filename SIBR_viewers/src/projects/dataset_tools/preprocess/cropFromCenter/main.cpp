...
*/
const char *USAGE =
    "Usage: cropFromCenter --inputFile <path_to_input_file> --outputPath <path_to_output_folder> --avgResolution "
    "<width x height> --cropResolution <width x height> [--scaleDownFactor <alpha> --targetResolution <width x "
    "height>] \n";
...
bool getParamas(int argc, const char **argv, std::string &inputFile, boost::filesystem::path &outputPath,
                sibr::Vector2i &avgResolution, sibr::Vector2i &cropResolution, float &scaleDownFactor,
    ...
    if (inputFile.empty() || outputFolder.empty() || avgResolution == sibr::Vector2i(0, 0)
...
int main(const int argc, const char **argv) {
    ...
    if (!getParamas(argc, argv, inputFileName, outputFolder, avgInitialResolution, cropResolution, scaleDownFactor,
                    targetResolution)) {
    ...
    for (int batchId = 0; batchId < nrBatches; batchId++) {
        const int nrItems = (batchId != nrBatches - 1) ? batchSize
                                                       : ((nrBatches * batchSize != int(pathToImgs.size()))
                                                              ? (int(pathToImgs.size()) - (batchSize * batchId))
                                                              : batchSize);
...
#pragma omp parallel for
        for (int localImgIndex = 0; localImgIndex < nrItems; localImgIndex++) {
            ...
            cv::Mat img = cv::imread(pathToImgs[globalImgIndex].string().c_str(), cv::IMREAD_COLOR);

            cv::Rect areOfIntererst = cv::Rect((img.cols - cropResolution[0]) / 2, (img.rows - cropResolution[1]) / 2,
                                               cropResolution[0], cropResolution[1]);

            cv::Mat croppedImg = img(areOfIntererst);

            cv::imwrite(outputFileName.c_str(), croppedImg);
            ...
            if (scaleDown) {
                ...
                cv::imwrite(scaledDownOutputFileName.c_str(), resizedImg);
    ...
    if (scaleDown) {
        ...
        if (targetResolution != sibr::Vector2i(0, 0)) {
            appUtility.writeTargetResolution((scaledDownOutputFolder / "target_resolution.txt").string(),
                                             targetResolution);
        }
    }
