...
void RenderMaskHolder::uploadMaskGPU(sibr::ImageL8 &img, int i, std::vector<RenderTargetLum::Ptr> &masks, bool invert) {
    ...
    textureShader.init("Texture", sibr::loadFile(sibr::Resources::Instance()->getResourceFilePathName("texture.vp")),
                       invert ? sibr::loadFile(sibr::getShadersDirectory("core") + "/texture-invert.frag")
...
void RenderMaskHolderArray::loadMasks(const sibr::BasicIBRScene::Ptr &ibrScene, const std::string &maskDir,
                                      const std::string &preFileName, const std::string &postFileName, int w, int h) {
    ...
    if (!boost::filesystem::exists(maskdir)) {
        ...
        for (int i = 0; i < numInputImgs; i++) {
            ...
            cv::Mat mask = cv::imread(filename.c_str(), cv::IMREAD_COLOR);
