...
void ImagesGrid::onUpdate(Input &input, const Viewport &vp) {
    ...
    if (current_level_tex) {
        ...
        imSizePixels = imSizePixels.cwiseQuotient(pow(2.0, current_lod) * Vector2f(1, 1)).unaryExpr([](float f) {
            return std::floor(f);
        });
...
void ImagesGrid::onRender(const Viewport &viewport) {
    ...
    draw_utils.image_grid(num_imgs, current_level_tex->handle(), grid_adjusted, viewRectangle.tl(), viewRectangle.br(),
                          current_lod, current_layer->flip_texture);
    ...
    if (ImGui::Begin("grid_gui")) {
        ...
        if (currentActivePix) {
            ...
            Vector4f value = current_layer->imgs_texture_array->readBackPixel(
                currentActivePix.im, currentActivePix.pos[0], currentActivePix.pos[1], current_lod);
...
void ImagesGrid::addImagesToHighlight(const std::string &name, const std::vector<int> &imgs, const Vector3f &col,
                                      float alpha_fill) {
    images_to_highlight[name] = { imgs, col, alpha_fill };
}

void ImagesGrid::addPixelsToHighlight(const std::string &name, const std::vector<MVpixel> &pixs, const Vector3f &col,
                                      float alpha_fill) {
...
void DrawUtilities::baseRendering(const Mesh &mesh, Mesh::RenderMode mode, const Vector3f &color,
                                  const Vector2f &translation, const Vector2f &scaling, float alpha,
                                  const Viewport &vp) {
    glEnable(GL_BLEND);
...
void DrawUtilities::rectangle(const Vector3f &color, const Vector2f &tl, const Vector2f &br, bool fill, float alpha,
    ...
    rectangleMesh->vertices(
        { { tl.x(), tl.y(), 0 }, { tl.x(), br.y(), 0 }, { br.x(), br.y(), 0 }, { br.x(), tl.y(), 0 } });
...
void DrawUtilities::rectanglePixels(const Vector3f &color, const Vector2f &center, const Vector2f &diagonalPixs,
                                    bool fill, float alpha, const Viewport &vp) {
...
void DrawUtilities::circle(const Vector3f &color, const Vector2f &center, float radius, bool fill, float alpha,
                           const Vector2f &scaling, int precision) {
...
void DrawUtilities::circlePixels(const Vector3f &color, const Vector2f &center, float radius, bool fill, float alpha,
                                 const Vector2f &winSize, int precision) {
...
void DrawUtilities::linePixels(const Vector3f &color, const Vector2f &ptA, const Vector2f &ptB,
...
void DrawUtilities::image_grid(int num_imgs, uint texture, const Vector2f &grid, const Vector2f &tl, const Vector2f &br,
                               int lod, bool flip_texture) {
    gridShader.begin();
...
void GridMapping::updateZoomBox(const Input &input, const sibr::Viewport &vp) {
    ...
    if (input.mouseButton().isPressed(Mouse::Code::Right) && !input.key().isActivated(Key::LeftControl)
        && !zoomSelection) {
        ...
        if (input.mouseButton().isReleased(Mouse::Code::Right)) {
            ...
            if (((currentBR - currentTL).array() > Vector2f(5, 5).array()).all()) {
                Vector2f tlPix = viewRectangle.tl().cwiseProduct(size)
                                 + (viewRectangle.br() - viewRectangle.tl()).cwiseProduct(currentTL);
                Vector2f brPix = viewRectangle.tl().cwiseProduct(size)
                                 + (viewRectangle.br() - viewRectangle.tl()).cwiseProduct(currentBR);
...
void GridMapping::highlightPixel(const MVpixel &pix, const Viewport &viewport, const Vector3f &color,
