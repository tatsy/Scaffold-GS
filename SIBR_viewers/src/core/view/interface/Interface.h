...
struct PixPos {
    ...
    PixPos(int i, const sibr::Vector2i &px)
        : im(i)
        , pos(px)
        , isDefined(true) {
    }
...
struct ScalingOptions {
    ScalingOptions()
        : numScale(1)
        , interpolation_method_cv(cv::INTER_CUBIC){};
    int numScale;
    int interpolation_method_cv;
};

struct LayerData {
    LayerData(const std::string &name)
        : name(name) {
    }
    std::string name;
};
...
public:
    ...
    /**
		...
		*/
    void displayLoop(
        sibr::Window &window, std::function<void(MultiViewInterface *)> = [](MultiViewInterface *) {});
...
public:
    ...
    void highlightPixel(const PixPos &pix, const sibr::Viewport &viewport, const sibr::Vector3f &color = { 0, 1, 0 },
...
#pragma omp parallel for
                for (int im = 0; im < (int)images.size(); ++im) {
                    ...
                    resized_imgs[im] =
                        images[im].resized(scaleSize[0], scaleSize[1], scalingOptions.interpolation_method_cv);
                }
            } else {
    ...
    void addImageLayerWithLambda(const std::vector<sibr::Image<T_Type, T_NumComp>> &images, LambdaType lambda,
