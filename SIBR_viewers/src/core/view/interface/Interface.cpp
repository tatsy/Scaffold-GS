...
void MultiViewInterface::updateImageView(const sibr::Viewport &viewport, const sibr::Input &input) {
    ...
    imgPixelScreenSize =
        screenPosPixelsFloat({ 0, { 1, 1 } }, winSize) - screenPosPixelsFloat({ 0, { 0, 0 } }, winSize);
...
UV01 MultiViewInterface::screenPosPixelCenter(const PixPos &pix) {
    sibr::Vector2f pos =
        ((pix.pos.cast<float>() + sibr::Vector2f(0.5, 0.5)).cwiseQuotient(scalesData[currentScale].imSize)
         + sibr::Vector2f(pix.im % grid.x(), pix.im / grid.x()))
            .cwiseQuotient(grid.cast<float>());
    pos.y() = 1.0f - pos.y();
    return UV01::from((pos - viewRectangle.tl()).cwiseQuotient(viewRectangle.br() - viewRectangle.tl()));
...
void MultiViewInterface::highlightPixel(const PixPos &pix, const sibr::Viewport &viewport, const sibr::Vector3f &color,
    ...
    if ((pixBR - pixTl).cwiseProduct(viewport.finalSize()).cwiseAbs().minCoeff() < 2.0f) {
        ...
        utils.rectanglePixels(color, 0.5f * (pixTl + pixBR).cwiseProduct(viewport.finalSize()), pixScreenSize, true,
                              0.15f, viewport.finalSize());
    } else {
...
void MultiViewInterface::displayImages(const sibr::Viewport &viewport) {
    ...
    if (reproMesh.get()) {
        ...
        for (int i = 0; i < (int)cams.size(); ++i) {
            ...
            sibr::Vector2i tlImgPix =
                screenPosPixels(PixPos(i, sibr::Vector2i(0, cams[i]->h() - 1)), viewPortSize.cast<float>());
            sibr::Vector2i brImgPix =
                screenPosPixels(PixPos(i, sibr::Vector2i(cams[i]->w() - 1, 0)), viewPortSize.cast<float>());
            ...
            glViewport(tlImgPix.x(), tlImgPix.y(), std::abs((brImgPix - tlImgPix).x()),
...
void MultiViewInterface::updateCurrentLayer(const sibr::Input &input) {
    ...
    std::vector<sibr::Key::Code> keys = { sibr::Key::Num1, sibr::Key::Num2, sibr::Key::Num3,
...
void MultiViewInterface::updateCenter(const sibr::Input &input, const sibr::Vector2f &winSize) {
    if (dclick.detected(input)) {
        ...
        sibr::Vector2f translation =
            (dclick.firstPosition.cast<float>().cwiseQuotient(winSize) - sibr::Vector2f(0.5, 0.5))
...
void MultiViewInterface::updateDrag(const sibr::Input &input, const sibr::Vector2f &winSize) {
    ...
    if (drag.isActive && input.mouseButton().isActivated(sibr::Mouse::Left)) {
        sibr::Vector2f translation = (input.mousePosition() - drag.position)
                                         .cast<float>()
