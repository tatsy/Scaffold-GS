...
struct UV : public sibr::Vector2f {
    static UV from(const sibr::Vector2f &v) {
        return UV(v.x(), v.y());
    }
...
public:
    ...
    void rectanglePixels(const sibr::Vector3f &color, const sibr::Vector2f &center, const sibr::Vector2f &diagonal,
                         bool fill, float alpha, const sibr::Vector2f &winSize);
    void circle(const sibr::Vector3f &color, const UV11 &center, float radius, bool fill, float alpha,
                const sibr::Vector2f &scaling = sibr::Vector3f(1, 1), int precision = 50);
    void circlePixels(const sibr::Vector3f &color, const sibr::Vector2f &center, float radius, bool fill, float alpha,
                      const sibr::Vector2f &winSize, int precision = 50);
    void linePixels(const sibr::Vector3f &color, const sibr::Vector2f &ptA, const sibr::Vector2f &ptB,
...
struct RectangleData {
    ...
    sibr::Vector2f br() const {
        return center + diagonal;
    }
    sibr::Vector2f tl() const {
        return center - diagonal;
    }
};
    ...
    bool detected(const sibr::Input &input, bool should_be_close = true) {
        if (input.mouseButton().isPressed(mKey)) {
            ...
            if (timer.deltaTimeFromLastTic<>() < detection_timing_in_ms
                && (!should_be_close || (firstPosition - input.mousePosition()).cwiseAbs().maxCoeff() < 10)) {
                return true;
            }
...
static void show(const sibr::Texture2DArray<T_Type, T_NumComp> &texArray, sibr::Window *inputWin = nullptr, int w = -1,
                 int h = -1) {
