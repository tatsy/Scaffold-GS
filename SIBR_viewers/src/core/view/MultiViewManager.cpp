...
void MultiViewBase::onUpdate(Input &input) {
    if (input.key().isActivated(Key::LeftControl) && input.key().isPressed(Key::LeftAlt)
        && input.key().isPressed(Key::P)) {
        _onPause = !_onPause;
    }
    if (_onPause) {
        return;
    ...
    _deltaTime =
        (float)(std::chrono::duration_cast<std::chrono::microseconds>(timeNow - _timeLastFrame).count()) / 1000000.0f;
    _timeLastFrame = timeNow;

    for (auto &subview : _subViews) {
        if (subview.second.view->active()) {
            auto subInput =
                !subview.second.view->isFocused() ? Input() : Input::subInput(input, subview.second.viewport, false);
...
void MultiViewBase::addSubView(const std::string &title, ViewBase::Ptr view, const Vector2u &res,
    ...
    const ViewUpdateFunc updateFunc = [](ViewBase::Ptr &vi, Input &in, const Viewport &vp, const float dt) {
        vi->onUpdate(in, vp);
    };
...
void MultiViewBase::addSubView(const std::string &title, ViewBase::Ptr view, const ViewUpdateFunc updateFunc,
    ...
    const Viewport viewport(0.0f, ImGui::GetTitleBarHeight(), res.x() > 0 ? res.x() : (float)_defaultViewResolution.x(),
                            (res.y() > 0 ? res.y() : (float)_defaultViewResolution.y()) + ImGui::GetTitleBarHeight());
    RenderTargetRGB::Ptr rtPtr(
        new RenderTargetRGB((uint)viewport.finalWidth(), (uint)viewport.finalHeight(), SIBR_CLAMP_UVS));
...
void MultiViewBase::renderSubView(SubView &subview) {
    if (!_onPause) {
        ...
        if (subview.handler != NULL
            && (subview.handler->getCamera().needVideoSave() || subview.handler->getCamera().needSave())) {
            ImageRGB frame;
    ...
    subview.view->setFocus(showImGuiWindow(subview.view->name(), *subview.rt, subview.flags, subview.viewport, false,
                                           subview.shouldUpdateLayout));
    ImGui::PopStyleVar();
...
MultiViewBase::BasicSubView::BasicSubView(ViewBase::Ptr view_, RenderTargetRGB::Ptr rt_, const sibr::Viewport viewport_,
...
MultiViewManager::MultiViewManager(Window &window, bool resize)
    ...
    , _fpsCounter(false) {
    if (resize) {
        window.size(Window::desktopSize().x() - 200, Window::desktopSize().y() - 200);
...
void MultiViewManager::onUpdate(Input &input) {
    ...
    if (input.key().isActivated(Key::LeftControl) && input.key().isActivated(Key::LeftAlt)
    ...
    if (_showGUI && ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("Menu")) {
            ...
            if (ImGui::MenuItem("Row layout")) {
                ...
                for (auto &view : _ibrSubViews) {
                    // Compute position on grid.
                    view.second.viewport = Viewport(vid * itemSize[0], verticalShift, (vid + 1.0f) * itemSize[0] - 1.0f,
            ...
            if (ImGui::MenuItem("Export Video")) {
                ...
                if (showFilePicker(saveFile, FilePickerMode::Save)) {
                    ...
                    if (!_videoFrames.empty()) {
                        ...
                        SIBR_WRG << "No frames to export!! Check save frames in camera options for the view you want "
