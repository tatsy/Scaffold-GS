...
void LabelsManager::setupLabelsManagerShader() {
    _labelShader.init("text-imgui", loadFile(Resources::Instance()->getResourceFilePathName("text-imgui.vp")),
...
SceneDebugView::SceneDebugView(const IIBRScene::Ptr &scene, const InteractiveCameraHandler::Ptr &camHandler,
...
SceneDebugView::SceneDebugView(const IIBRScene::Ptr &scene, const Viewport &viewport,
    ...
    SIBR_WRG << "Deprecated SceneDebugView constructor, use the version without viewport passed as argument."
             << std::endl;
}
...
void SceneDebugView::onRender(const Viewport &viewport) {
    ...
    addMeshAsLines("scene cam", generateCamFrustum(_userCurrentCam->getCamera(), 0.0f, _cameraScaling))
        .setColor({ 1, 0, 0 });

    if (_scene) {
        for (int i = 0; i < (int)_scene->cameras()->inputCameras().size(); ++i) {
            _cameras[i].highlight =
                _scene->cameras()->isCameraUsedForRendering(_scene->cameras()->inputCameras()[i]->id());
        }
    }
    ...
    for (const auto &camInfos : _cameras) {
        ...
        (camInfos.highlight ? used_cams : non_used_cams)
            ->merge(*generateCamFrustum(camInfos.cam, 0.0f, _cameraScaling));
    }
        ...
        for (const auto &camInfos : _cameras) {
            if (camInfos.cam.isActive()) {
                ...
                if (scene_rts->getInputRGBTextureArrayPtr()) {
                    renderImage(camera_handler.getCamera(), camInfos.cam,
                                scene_rts->getInputRGBTextureArrayPtr()->handle(), cam_id);
                } else {
...
void SceneDebugView::setScene(const IIBRScene::Ptr &scene, bool preserveCamera) {
    ...
    setup();
    camera_handler.setup(_scene->cameras()->inputCameras(), camera_handler.getViewport(),
...
void SceneDebugView::setup() {
    if (_scene) {
        ...
        for (const auto &inputCam : _scene->cameras()->inputCameras()) {
            _cameras.push_back(
                CameraInfos(*inputCam, inputCam->id(), _scene->cameras()->isCameraUsedForRendering(inputCam->id())));
        }
    }
...
void SceneDebugView::setupMeshes() {
    ...
    if (!_scene->proxies()->proxyPtr()->hasColors() && !_scene->proxies()->proxyPtr()->hasTexCoords()) {
        ...
        if (fileExists(fn = _scene->data()->basePathName() + "/capreal/mesh.ply")) {
            if (sdv_mesh.load(fn, _scene->data()->basePathName())) success = true;
        }
        // in sibr subdir
