...
void InteractiveCameraHandler::setup(const std::vector<InputCamera::Ptr> &cams, const sibr::Vector2u &resolution,
                                     const sibr::Viewport &viewport, const std::shared_ptr<sibr::Raycaster> raycaster) {
    setup(cams, viewport, raycaster);
}

void InteractiveCameraHandler::setup(const sibr::InputCamera &cam, const sibr::Viewport &viewport,
...
void InteractiveCameraHandler::setup(const Eigen::AlignedBox<float, 3> &areaOfInterest, const sibr::Viewport &viewport,
                                     const std::shared_ptr<sibr::Raycaster> raycaster) {
    _raycaster = raycaster;
    _viewport = viewport;
...
void InteractiveCameraHandler::setup(const std::vector<InputCamera::Ptr> &cams, const sibr::Viewport &viewport,
                                     const std::shared_ptr<sibr::Raycaster> raycaster,
    ...
    SIBR_LOG << "Interactive camera using (" << idealCam.znear() << "," << idealCam.zfar() << ") near/far planes."
             << std::endl;
...
int InteractiveCameraHandler::findNearestCamera(const std::vector<InputCamera::Ptr> &inputCameras) const {
    ...
    std::vector<uint> sortByDistance =
        sibr::IBRBasicUtils::selectCamerasSimpleDist(inputCameras, _currentCamera, numCams);
    std::vector<uint> sortByAngle =
        sibr::IBRBasicUtils::selectCamerasAngleWeight(inputCameras, _currentCamera, numCams);
...
void InteractiveCameraHandler::setupInterpolationPath(const std::vector<InputCamera::Ptr> &cameras) {
    ...
    if (defaultPath) {
        ...
        std::sort(_interpPath.begin(), _interpPath.end(),
                  [](const InputCamera::Ptr &a, const InputCamera::Ptr &b) { return a->id() < b->id(); });
    }
}
...
void InteractiveCameraHandler::update(const sibr::Input &input, float deltaTime, const sibr::Viewport &viewport) {
    ...
    if (input.key().isReleased(sibr::Key::B)) {
        ...
        if (_currentMode == TRACKBALL) {
            ...
            SIBR_LOG << "[Trackball] Display visual guides: " << (_trackball.drawThis ? "on" : "off") << "."
                     << std::endl;
        }
        ...
        if (input.key().isActivated(Key::LeftShift)
            && (input.key().isActivated(Key::LeftAlt) || input.key().isActivated(Key::LeftControl))
    ...
    if (!_cameraRecorder.isPlaying()) {
        ...
        if (_shouldSmooth && _currentMode != INTERPOLATION) {
            const sibr::Camera newcam =
                sibr::Camera::interpolate(_previousCamera, _currentCamera, IBRVIEW_SMOOTHCAM_POWER);
...
void InteractiveCameraHandler::onGUI(const std::string &suffix) {
    ...
    if (ImGui::Begin(fullName.c_str())) {
        ...
        ImGui::Text("Key cameras: %zu", _keyCameras.size());
        ImGui::SameLine();
