    ...
    if (ImGui::BeginPopup(("##Options_popup_" + name).c_str())) {
        ...
        if (meshType == TRIANGLES) {
            ...
            if (showNormals) {
                ...
                ImGui::PopItemWidth();
                ImGui::ColorEdit3(("NormalsColor##color_picker_" + name).c_str(), &normalsColor[0],
...
void ShaderAlphaMVP::initShader(const std::string &name, const std::string &vert, const std::string &frag,
...
void ColorMeshShader::initShader(const std::string &name, const std::string &vert, const std::string &frag,
                                 const std::string &geom) {
...
void PointShader::initShader(const std::string &name, const std::string &vert, const std::string &frag,
                             const std::string &geom) {
...
void NormalRenderingShader::initShader(const std::string &name, const std::string &vert, const std::string &frag,
                                       const std::string &geom) {
    ColorMeshShader::initShader(name, vert, frag, geom);
...
void MeshShadingShader::initShader(const std::string &name, const std::string &vert, const std::string &frag,
                                   const std::string &geom) {
    ColorMeshShader::initShader(name, vert, frag, geom);
...
MultiMeshManager::MultiMeshManager(const std::string &_name)
    : name(_name) {
    initShaders();
...
void MultiMeshManager::initShaders() {
    ...
    colored_mesh_shader.initShader("colored_mesh_shader", loadFile(folder + "alpha_colored_mesh.vert"),
                                   loadFile(folder + "alpha_colored_mesh.frag"));
    points_shader.initShader("points_shader", loadFile(folder + "alpha_points.vert"),
                             loadFile(folder + "alpha_points.frag"));
    per_vertex_normals_shader.initShader(
        "per_vertex_normal_shader", loadFile(folder + "alpha_colored_per_vertex_normals.vert"),
        loadFile(folder + "alpha_colored_mesh.frag"), loadFile(folder + "alpha_colored_per_vertex_normals.geom"));
    per_triangle_normals_shader.initShader(
        "per_triangle_normal_shader", loadFile(folder + "alpha_colored_per_triangle_normals.vert"),
        loadFile(folder + "alpha_colored_mesh.frag"), loadFile(folder + "alpha_colored_per_triangle_normals.geom"));
}
...
void MultiMeshManager::list_mesh_onGUI() {
    ...
    if (ImGui::CollapsingHeader(("Meshes list##" + name).c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::Button("load Mesh##MeshesList")
            && showFilePicker(loaded_mesh_path, FilePickerMode::Default, "", "obj,ply")) {
...
MeshData &MultiMeshManager::addMesh(const std::string &name, Mesh::Ptr mesh, Raycaster::Ptr raycaster,
...
MeshData &MultiMeshManager::addLines(const std::string &name, const std::vector<Vector3f> &endPoints,
...
MeshData &MultiMeshManager::addPoints(const std::string &name, const std::vector<Vector3f> &points,
                                      const Vector3f &color) {
