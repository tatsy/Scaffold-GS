...
public:
    ...
    typedef std::function<void(sibr::ViewBase::Ptr &, sibr::Input &, const sibr::Viewport &, const float)>
        ViewUpdateFunc;
    /// Update callback for an IBR view, see main description for the return value.
    typedef std::function<sibr::InputCamera(sibr::ViewBase::Ptr &, sibr::Input &, const sibr::Viewport &, const float)>
        IBRViewUpdateFunc;
    /// Additional render callback for a subview.
    typedef std::function<void(sibr::ViewBase::Ptr &, const sibr::Viewport &, const IRenderTarget::Ptr &)>
        AdditionalRenderFunc;
    ...
    /**
		 ...
		 */
    void addSubView(const std::string &title, ViewBase::Ptr view, const Vector2u &res = Vector2u(0, 0),
    ...
    /**
		...
		* \param flags ImGui_WindowFlags to pass to the internal window manager.
		*/
    void addSubView(const std::string &title, ViewBase::Ptr view, const ViewUpdateFunc updateFunc,
                    const Vector2u &res = Vector2u(0, 0), const ImGuiWindowFlags flags = 0);
    ...
    /**
		...
		* \param flags ImGui_WindowFlags to pass to the internal window manager.
		*/
    void addIBRSubView(const std::string &title, ViewBase::Ptr view, const Vector2u &res = Vector2u(0, 0),
                       const ImGuiWindowFlags flags = 0);
    ...
    /**
		...
		* \param flags ImGui_WindowFlags to pass to the internal window manager.
		*/
    void addIBRSubView(const std::string &title, ViewBase::Ptr view, const IBRViewUpdateFunc updateFunc,
                       const Vector2u &res = Vector2u(0, 0), const ImGuiWindowFlags flags = 0);
    ...
    /**
		...
		*/
    void captureView(const std::string &subviewName, const std::string &path = "./screenshots",
...
protected:
    ...
    struct SubView {
        ...
        /** Constructor.
			 ...
			 */
        SubView(ViewBase::Ptr view_, RenderTargetRGB::Ptr rt_, const sibr::Viewport viewport_, const std::string &name_,
...
protected:
    ...
    /** Capture a view as an image on disk.
		 ...
		 **/
    static void captureView(const SubView &view, const std::string &path = "./screenshots/",
                            const std::string &filename = "");
