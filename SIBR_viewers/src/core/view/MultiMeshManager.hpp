...
public:
    /** Initialize the shader.
		 ...
		 *\param geom the geometry shader content
		 */
    void initShader(const std::string &name, const std::string &vert, const std::string &frag,
                    const std::string &geom = "") override;
...
public:
    ...
    /** COnstructor.
		 ...
		 */
    MeshData(const std::string &_name = "", Mesh::Ptr mesh_ptr = {}, MeshType mesh_type = TRIANGLES,
             Mesh::RenderMode render_mode = Mesh::FillRenderMode);
...
public:
    ...
    /** Add lines to the visualization, defined by their endpoints.
		...
		\return a reference to the object display options (for chained modifications).
		*/
    MeshData &addLines(const std::string &name, const std::vector<Vector3f> &endPoints,
                       const Vector3f &color = { 0, 1, 0 });
    ...
    /** Add points to the visualization.
		...
		*/
    MeshData &addPoints(const std::string &name, const std::vector<Vector3f> &points,
                        const Vector3f &color = { 1, 0, 0 });
    ...
    InteractiveCameraHandler &getCameraHandler() {
        return camera_handler;
    }

    /** \return the colored mesh shader */
    MeshShadingShader &getMeshShadingShader() {
        return colored_mesh_shader;
    }
...
protected:
    ...
    NormalRenderingShader per_vertex_normals_shader,
        per_triangle_normals_shader;  ///< Shaders for visualizing an object normals.
