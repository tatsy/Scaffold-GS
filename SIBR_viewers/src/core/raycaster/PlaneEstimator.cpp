...
void PlaneEstimator::computePlanes(const int numPlane, const float delta, const int numTry) {
    ...
    for (int i = 0; i < numPlane; i++) {
        ...
        sibr::Vector3f color(((float)rand() / float(RAND_MAX)), ((float)rand() / float(RAND_MAX)),
        ...
        std::cout << "vote :" << vote << " notSel " << notSel << " supposed total " << _remainPoints3D.rows()
                  << std::endl;
...
sibr::Vector4f PlaneEstimator::estimatePlane(const float delta, const int numTry, Eigen::MatrixXi &bestMask,
                                             int &bestVote, std::pair<Eigen::MatrixXf, sibr::Vector3f> &bestCovMean) {
...
#pragma omp critical
    ...
    std::cout << "Vote refined " << bestVote << " Plane refined " << bestPlane
              << /*" Normals coherency " << bestMaskNormals.sum()/bestVote  <<*/ std::endl;
...
std::pair<int, float> PlaneEstimator::votePlane(const sibr::Vector4f plane, const float delta, Eigen::MatrixXi &mask,
                                                float normalDot) {
