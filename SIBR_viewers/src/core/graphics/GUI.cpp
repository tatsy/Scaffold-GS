...
bool showImGuiWindow(const std::string &windowTitle, const IRenderTarget &rt, ImGuiWindowFlags flags,
                     Viewport &viewport, bool invalidTexture, bool updateLayout, int handle) {
    ...
    if (::ImGui::Begin(windowTitle.c_str(), NULL, flags)) {
        ...
        if (!invalidTexture) {
            ::ImGui::GetWindowDrawList()->AddImage((void *)(intptr_t)(rt.handle(handle)), pos,
...
sibr::Vector2f ZoomData::topLeft() const {
    return center - diagonal;
}
sibr::Vector2f ZoomData::bottomRight() const {
    return center + diagonal;
}
...
void ZoomInterraction::updateZoom(const sibr::Vector2f &canvasSize) {
    ...
    if (zoomData.underMofidication) {
        ImGui::GetWindowDrawList()->AddRect(ImVec2(zoomData.firstClickPixel[0], zoomData.firstClickPixel[1]),
...
void DisplayImageGui(GLuint texture, const sibr::Vector2i &displaySize, const sibr::Vector2f &uv0,
                     const sibr::Vector2f &uv1) {
    ImGui::Image((void *)(intptr_t)(texture), ImVec2(float(displaySize[0]), float(displaySize[1])),
                 ImVec2(uv0[0], uv0[1]), ImVec2(uv1[0], uv1[1]));
}

void ImageWithCallback(GLuint texture, const sibr::Vector2i &displaySize, CallBackData &callbackDataOut,
                       const sibr::Vector2f &uv0, const sibr::Vector2f &uv1) {
...
void PlotMultiLines(const char *label, std::vector<float *> values, int values_count, const std::vector<ImVec4> &colors,
