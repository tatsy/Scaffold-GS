...
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<uint8>(void) {
    return CV_8U;
}
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<int8>(void) {
    return CV_8S;
}
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<uint16>(void) {
    return CV_16U;
}
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<int16>(void) {
    return CV_16S;
}
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<int32>(void) {
    return CV_32S;
}
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<float>(void) {
    return CV_32F;
}
/** \return the OpenCV type corresponding to a C type. */
template <>
SIBR_GRAPHICS_EXPORT inline int imageType<double>(void) {
    return CV_64F;
}
...
/** \return the size of the range of values a type can take when used in OpenCV. */
template <>
SIBR_GRAPHICS_EXPORT inline float imageTypeRange<float>(void) {
    return 1.f;
}
/** \return the size of the range of values a type can take when used in OpenCV. */
template <>
SIBR_GRAPHICS_EXPORT inline float imageTypeRange<double>(void) {
    return 1.f;
}
...
public:
    ...
    /** Check if a pixel (x,y) is inside the image boundaries.
		\param xy the pixel coordinates
		\return true if 0<=x<w and 0<=y=<h */
    bool isInRange(const sibr::Vector2i &xy) const {
        return (xy.x() >= 0 && xy.y() >= 0 && xy.x() < (int)w() && xy.y() < (int)h());
    }

    /** Check if a pixel (x,y) is inside the image boundaries.
		\param xy the pixel coordinates
		\return true if 0<=x<w and 0<=y=<h */
    bool isInRange(const sibr::Vector2f &xy) const {
        return (xy.x() >= 0 && xy.y() >= 0 && xy.x() < (float)w() && xy.y() < (float)h());
    }
    ...
    int opencvType(void) const {
        return CV_MAKETYPE(opencv::imageType<T_Type>(), T_NumComp);
    }

    /** \copydoc IImage::toOpenCV */
    const cv::Mat &toOpenCV(void) const {
        return _pixels;
    }

    /** \copydoc IImage::toOpenCVnonConst */
    cv::Mat &toOpenCVnonConst(void) {
        return _pixels;
    }
...
protected:
    /** Helper for bicubic interpolation.
		...
		*/
    static Eigen::Matrix<float, T_NumComp, 1, Eigen::DontAlign> monoCubic(
        float t, const Eigen::Matrix<float, T_NumComp, 4, Eigen::DontAlign> &colors);
...
public:
    ...
    /// Default constructor.
    ImagePtr() {
        imPtr = std::shared_ptr<Image<T_Type, T_NumComp>>();
    };

    /** Constructor from a raw pointer.
		\param imgPtr the raw pointer to wrap
		*/
    ImagePtr(Image<T_Type, T_NumComp> *imgPtr) {
        imPtr = std::shared_ptr<Image<T_Type, T_NumComp>>(imgPtr);
    };

    /** Constructor from a shared pointer.
		\param imgPtr the shared pointer to wrap
		*/
    ImagePtr(const std::shared_ptr<Image<T_Type, T_NumComp>> &imgPtr) {
        imPtr = std::shared_ptr<Image<T_Type, T_NumComp>>(imgPtr);
    };
    ...
    /** Generate a pointer by cloning an image.
		...
		\return the pointer
		*/
    static ImagePtr fromImg(const ImageType &img) {
        return ImagePtr(std::make_shared<Image<T_Type, T_NumComp>>(img.clone()));
    };

    /** Set a new pointee.
		\param ptr the new image pointer
		*/
    void reset(ImageType *ptr) {
        imPtr.reset(ptr);
    };

    /** \return the image */
    Image<T_Type, T_NumComp> *get() {
        return imPtr.get();
    };
    ...
    Image<T_Type, T_NumComp> &operator*() {
        return imPtr.operator*();
    };

    /** \return the dereferenced image */
    const Image<T_Type, T_NumComp> &operator*() const {
        return imPtr.operator*();
    };
    ...
    /** Assign a shared ptr.
		...
		*/
    std::shared_ptr<Image<T_Type, T_NumComp>> &operator=(std::shared_ptr<Image<T_Type, T_NumComp>> &imgShPtr) {
        imPtr = imgShPtr;
        return &imPtr;
    };
...
/** Display a 32F image in a debug window, using the Parula colormap after normalizing the values.
	...
	*/
SIBR_GRAPHICS_EXPORT void showFloat(const Image<float, 1> &im, bool logScale = false, double min = -DBL_MAX,
                                    double max = DBL_MAX);
...
static void show(const Image<T_Type, T_NumComp> &img, const std::string &windowTitle = "sibr::show()",
...
Image<T_Type, T_NumComp>::Image(uint width, uint height, const Pixel &init) {
    cv::Scalar scal(0);
    for (int i = 0; i < T_NumComp; i++) scal(i) = init(i);
    ...
    if (img.channels() != T_NumComp) {
        ...
        for (int y = 0; y < img.rows; ++y) {
            for (int x = 0; x < img.cols; ++x) {
                ...
                assert(ptr != nullptr);
                uint i;
                for (i = 0; i < (uint)img.channels() && i < T_NumComp; ++i) p[i] = ptr[i];
                for (; i < T_NumComp && i < 3; ++i) p[i] = p[0];
                for (; i < T_NumComp && i < 4; ++i) p[i] = static_cast<T_Type>(opencv::imageTypeRange<T_Type>());
...
bool Image<T_Type, T_NumComp>::load(const std::string &filename, bool verbose, bool warning_if_not_found) {
    if (verbose)
        ...
        std::cerr << ".";
    cv::Mat img = cv::imread(filename.c_str(), cv::IMREAD_UNCHANGED | cv::IMREAD_ANYDEPTH | cv::IMREAD_ANYCOLOR);
    if (img.data == nullptr) {
...
bool Image<T_Type, T_NumComp>::loadByteStream(const std::string &filename, bool verbose) {
    ...
    sibr::ByteStream bs;
    if (!bs.load(filename)) SIBR_WRG << "Image file not found '" << filename << "'." << std::endl;
...
void Image<T_Type, T_NumComp>::save(const std::string &filename, bool verbose) const {
    {  // Create the output dir if doesn't exists
        ...
        if (outdir.empty() == false) boost::filesystem::create_directories(outdir);
    }
    ...
    if (verbose) SIBR_LOG << "Saving image file '" << filename << "'." << std::endl;
    ...
    if (img.cols > 0 && img.rows > 0) {
        if (cv::imwrite(filename.c_str(), finalImage) == false)
...
template <typename T_Type, unsigned int T_NumComp>
inline const typename Image<T_Type, T_NumComp>::Pixel &Image<T_Type, T_NumComp>::operator()(
    const sibr::Vector2i &xy) const {
    return operator()(xy[0], xy[1]);
}
template <typename T_Type, unsigned int T_NumComp>
inline const typename Image<T_Type, T_NumComp>::Pixel &ImagePtr<T_Type, T_NumComp>::operator()(
    const sibr::Vector2i &xy) const {
...
ColorRGBA Image<T_Type, T_NumComp>::color(uint x, uint y) const {
    ...
    return ColorRGBA(v.val[0] * scale, v.val[1] * scale, v.val[2] * scale, (T_NumComp > 3) ? v.val[3] * scale : 1.f);
}
template <typename T_Type, unsigned int T_NumComp>
...
Image<T_Type, T_NumComp> Image<T_Type, T_NumComp>::resized(int width, int height, int cv_interpolation_method) const {
    if (width == w() && height == h()) return clone();
    Image dst;
...
template <typename T_Type, unsigned int T_NumComp>
Eigen::Matrix<T_Type, T_NumComp, 1, Eigen::DontAlign> Image<T_Type, T_NumComp>::bilinear(
    const sibr::Vector2f &queryPosition) const {
    if (w() < 2 || h() < 2) {
        return Eigen::Matrix<T_Type, T_NumComp, 1, Eigen::DontAlign>();
    }

    const sibr::Vector2i cornerPixel =
        sibr::Vector2f((queryPosition - 0.5f * sibr::Vector2f(1, 1)).unaryExpr([](float f) {
            return std::floor(f);
        })).template cast<int>();
    ...
    return (operator()(mm).template cast<float>() * (1.0f - ts[0]) * (1.0f - ts[1])
...
template <typename T_Type, unsigned int T_NumComp>
Eigen::Matrix<float, T_NumComp, 1, Eigen::DontAlign> Image<T_Type, T_NumComp>::monoCubic(
    float t, const Eigen::Matrix<float, T_NumComp, 4, Eigen::DontAlign> &colors) {
    static const Eigen::Matrix<float, 4, 4> M =
        0.5f
        * (Eigen::Matrix<float, 4, 4>() << 0, 2, 0, 0, -1, 0, 1, 0, 2, -5, 4, -1, -1, 3, -3, 1).finished().transpose();

    return colors * (M * Eigen::Matrix<float, 4, 1>(1, t, t * t, t * t * t));
}

template <typename T_Type, unsigned int T_NumComp>
Eigen::Matrix<T_Type, T_NumComp, 1, Eigen::DontAlign> Image<T_Type, T_NumComp>::bicubic(
    const sibr::Vector2f &queryPosition) const {
    static const std::vector<std::vector<sibr::Vector2i>> offsets = { { { -1, -1 }, { 0, -1 }, { 1, -1 }, { 2, -1 } },
                                                                      { { -1, 0 }, { 0, 0 }, { 1, 0 }, { 2, 0 } },
                                                                      { { -1, 1 }, { 0, 1 }, { 1, 1 }, { 2, 1 } },
                                                                      { { -1, 2 }, { 0, 2 }, { 1, 2 }, { 2, 2 } } };
    ...
    const sibr::Vector2i cornerPixel = (queryPosition - 0.5f * sibr::Vector2f(1, 1))
    ...
    return (resultFloat.unaryExpr([](float f) { return sibr::clamp(f, 0.0f, sibr::opencv::imageTypeRange<T_Type>()); }))
        .template cast<T_Type>();
}
