...
InputCamera::InputCamera(int id, int w, int h, sibr::Vector3f &position, sibr::Matrix3f &orientation, float focal,
                         float k1, float k2, bool active)
    : _active(active) {
    _w = w;
    _h = h;
...
InputCamera::InputCamera(const Camera &c, int w, int h)
    : Camera(c) {
...
uint InputCamera::w(void) const {
    return _w;
}
uint InputCamera::h(void) const {
    return _h;
}
bool InputCamera::isActive(void) const {
    return _active;
}
...
float InputCamera::focal() const {
    return _focal;
};
float InputCamera::focalx() const {
    return _focalx;
};
float InputCamera::k1() const {
    return _k1;
};
float InputCamera::k2() const {
    return _k2;
};
...
std::vector<InputCamera::Ptr> InputCamera::load(const std::string &datasetPath, float zNear, float zFar,
                                                const std::string &bundleName, const std::string &listName) {
...
std::vector<InputCamera::Ptr> InputCamera::loadNVM(const std::string &nvmPath, float zNear, float zFar,
    ...
    if (in.is_open()) {
        ...
        for (int i = 0; i < ncam; ++i) {
            ...
            if (format_r9t) {
                std::cout << " WARNING THIS PART OF THE CODE WAS NEVER TESTED. IT IS SUPPOSED NOT TO WORK PROPERLY"
                          << std::endl;
                Eigen::Matrix3f matRotation;
                matRotation << float(q[0]), float(q[1]), float(q[2]), float(q[3]), float(q[4]), float(q[5]),
...
std::vector<InputCamera::Ptr> InputCamera::loadLookat(const std::string &lookatPath,
                                                      const std::vector<sibr::Vector2u> &wh, float znear, float zfar) {
    ...
    if (in.is_open()) {
        ...
        for (std::string line; safeGetline(in, line); i++) {
            ...
            Vector3f Eye(std::strtof(vecVal[0].c_str(), 0), std::strtof(vecVal[1].c_str(), 0),
                         std::strtof(vecVal[2].c_str(), 0));
            boost::split(vecVal, targetStr, [](char c) { return c == ','; });
            Vector3f At(std::strtof(vecVal[0].c_str(), 0), std::strtof(vecVal[1].c_str(), 0),
                        std::strtof(vecVal[2].c_str(), 0));

            boost::split(vecVal, upStr, [](char c) { return c == ','; });
            Vector3f Up(std::strtof(vecVal[0].c_str(), 0), std::strtof(vecVal[1].c_str(), 0),
                        std::strtof(vecVal[2].c_str(), 0));
...
std::string InputCamera::lookatString() const {
    std::string infos = std::string(" -D origin=") + std::to_string(position()[0]) + std::string(",")
                        + std::to_string(position()[1]) + std::string(",") + std::to_string(position()[2])
                        + std::string(" -D target=") + std::to_string(position()[0] + dir()[0]) + std::string(",")
                        + std::to_string(position()[1] + dir()[1]) + std::string(",")
                        + std::to_string(position()[2] + dir()[2]) + std::string(" -D up=") + std::to_string(up()[0])
                        + std::string(",") + std::to_string(up()[1]) + std::string(",") + std::to_string(up()[2])
                        + std::string(" -D fovy=") + std::to_string(180 * fovy() / M_PI) + std::string(" -D clip=")
                        + std::to_string(znear()) + std::string(",") + std::to_string(zfar()) + std::string("\n");
    return infos;
}
...
std::vector<InputCamera::Ptr> InputCamera::loadColmap(const std::string &colmapSparsePath, const float zNear,
                                                      const float zFar, const int fovXfovYFlag) {
    ...
    while (safeGetline(imagesFile, line)) {
        ...
        if (fovXfovYFlag) {
            camera = std::make_shared<InputCamera>(InputCamera(camParams.fy, camParams.fx, 0.0f, 0.0f,
                                                               int(camParams.width), int(camParams.height), int(cId)));
        } else {
            camera = std::make_shared<InputCamera>(
                InputCamera(camParams.fy, 0.0f, 0.0f, int(camParams.width), int(camParams.height), int(cId)));
        }
        ...
        if (camera->position().x() < 0) {
            camerasFile2 << ++valid;
            for (int i = 1; i < camidtokens[id].size(); i++) camerasFile2 << " " << camidtokens[id][i];
            camerasFile2 << "\n\n";

            imagesFile2 << valid;
            for (int i = 1; i < tokens.size() - 1; i++) imagesFile2 << " " << tokens[i];
...
std::vector<InputCamera::Ptr> InputCamera::loadBundle(const std::string &bundlerPath, float zNear, float zFar,
...
std::vector<InputCamera::Ptr> InputCamera::loadBundleFRIBR(const std::string &bundlerPath, float zNear, float zFar,
    ...
    for (int i = 0; i < numImages; i++) {
        ...
        cameras[i] = InputCamera::Ptr(
            new InputCamera(i, resolution.x(), resolution.y(), position, orientation, f, k1, k2, true));
...
std::vector<InputCamera::Ptr> InputCamera::loadMeshroom(const std::string &meshroomSFMPath, const float zNear,
                                                        const float zFar) {
    ...
    for (size_t i = 0; i < numCameras; ++i) {
        ...
        float dx =
            std::stof(intrinsincs[intrinsic_idx].get("principalPoint").get<picojson::array>()[0].get<std::string>());
        float dy =
            std::stof(intrinsincs[intrinsic_idx].get("principalPoint").get<picojson::array>()[1].get<std::string>());
        ...
        Eigen::Vector3f row;
        Eigen::Vector3f position(std::stof(center[0].get<std::string>()), std::stof(center[1].get<std::string>()),
                                 std::stof(center[2].get<std::string>()));
        Eigen::Matrix3f orientation;

        for (int ii = 0; ii < 3; ++ii) {
            for (int jj = 0; jj < 3; ++jj) row(jj) = std::stof(rotation[jj + ii * 3].get<std::string>());
            rows.push_back(row);
        }
...
Vector3f InputCamera::unprojectImgSpaceInvertY(const sibr::Vector2i &pixelPos, const float &depth) const {
    sibr::Vector2f pos2dGL(
        2.0f * ((pixelPos.cast<float>() + sibr::Vector2f(0.5, 0.5)).cwiseQuotient(sibr::Vector2f(w(), h())))
...
bool InputCamera::loadFromBinary(const std::string &filename) {
    ...
    if (bytes.load(filename)) {
        ...
        bytes >> focal >> k1 >> k2 >> w >> h >> pos.x() >> pos.y() >> pos.z() >> rot.w() >> rot.x() >> rot.y()
            >> rot.z() >> fov >> aspect >> znear >> zfar;
...
void InputCamera::saveToBinary(const std::string &filename) const {
    ...
    bytes << version << focal << k1 << k2 << w << h << pos.x() << pos.y() << pos.z() << rot.w() << rot.x() << rot.y()
          << rot.z() << fov << aspect << znear << zfar;
...
void InputCamera::saveAsBundle(const std::vector<InputCamera::Ptr> &cams, const std::string &fileName, bool negativeZ,
    ...
    if (exportImages) {
        ...
        if (outList.good()) {
            for (int i = 0; i < cams.size(); ++i) {
                ...
                cv::imwrite((imagesDir + imageName).c_str(), dummy);
            }
            outList.close();
        } else {
...
std::vector<InputCamera::Ptr> InputCamera::loadColmapBin(const std::string &colmapSparsePath, const float zNear,
                                                         const float zFar, const int fovXfovYFlag) {
...
std::vector<InputCamera::Ptr> InputCamera::loadJSON(const std::string &jsonPath, const float zNear, const float zFar) {
    ...
    for (size_t i = 0; i < frames.size(); ++i) {
        ...
        sibr::InputCamera::Ptr camera =
            std::make_shared<InputCamera>(InputCamera(fy, fx, 0.0f, 0.0f, width, height, id));
...
std::vector<InputCamera::Ptr> InputCamera::loadCityTransform(const std::string &transformPath, const float zNear,
                                                             const float zFar, const int fovXfovYFlag) {
...
std::vector<InputCamera::Ptr> InputCamera::loadTransform(const std::string &transformPath, int w, int h,
                                                         std::string extension, const float zNear, const float zFar,
                                                         const int offset, const int fovXfovYFlag) {
    std::ifstream json_file(transformPath, std::ios::in);
