...
public:
    ...
    /** Constructor, initialize the input camera.
		...
		*/
    InputCamera(int id, int w, int h, sibr::Vector3f &position, sibr::Matrix3f &rotation, float focal, float k1,
    ...
    /** Save a vector of cameras as a bundle file.
		...
		*/
    static void saveAsBundle(const std::vector<InputCamera::Ptr> &cams, const std::string &fileName,
                             bool negativeZ = false, bool exportImages = false, bool recomputeFocal = true);
    ...
    /** Load cameras from a bundler file.
		 ...
		 */
    static std::vector<InputCamera::Ptr> load(const std::string &datasetPath, float zNear = 0.01f, float zFar = 1000.0f,
                                              const std::string &bundleName = "bundle.out",
                                              const std::string &listName = "list_images.txt");
    ...
    /** Load cameras from a NVM file.
		...
		* \returns the loaded cameras
		*/
    static std::vector<InputCamera::Ptr> loadNVM(const std::string &nvmPath, float zNear = 0.01f, float zFar = 1000.0f,
                                                 std::vector<sibr::Vector2u> wh = std::vector<sibr::Vector2u>());
    ...
    /** Load cameras from a .lookat file generated by our Blender plugin.
		...
		* \returns the loaded cameras
		*/
    static std::vector<InputCamera::Ptr> loadLookat(
        const std::string &lookatPath, const std::vector<sibr::Vector2u> &wh = std::vector<sibr::Vector2u>(),
        float zNear = -1, float zFar = -1);

    static std::vector<InputCamera::Ptr> loadTransform(const std::string &transformPath, int w, int h,
                                                       std::string extension, const float zNear = 0.01f,
                                                       const float zFar = 1000.0f, const int offset = 0,
                                                       const int fovXfovYFlag = 0);

    static std::vector<InputCamera::Ptr> loadCityTransform(const std::string &transformPath, const float zNear = 0.01f,
                                                           const float zFar = 1000.0f, const int fovXfovYFlag = 0);
    /** Load cameras from a Colmap txt file.
		...
		*/
    static std::vector<InputCamera::Ptr> loadColmap(const std::string &colmapSparsePath, const float zNear = 0.01f,
                                                    const float zFar = 1000.0f, const int fovXfovYFlag = 0);

    static std::vector<InputCamera::Ptr> loadColmapBin(const std::string &colmapSparsePath, const float zNear = 0.01f,
                                                       const float zFar = 1000.0f, const int fovXfovYFlag = 0);

    static std::vector<InputCamera::Ptr> loadJSON(const std::string &jsonPath, const float zNear = 0.01f,
    ...
    /** Load cameras from a bundle file.
		...
		* \returns the loaded cameras
		*/
    static std::vector<InputCamera::Ptr> loadBundle(const std::string &bundlerPath, float zNear = 0.01f,
                                                    float zFar = 1000.0f, const std::string &listImagePath = "",
                                                    bool path = false);
    ...
    /** Load cameras from a bundle file.
		...
		* \returns the loaded cameras
		*/
    static std::vector<InputCamera::Ptr> loadBundleFRIBR(const std::string &bundlerPath, float zNear = 0.01f,
                                                         float zFar = 1000.0f, const std::string &listImagePath = "");
    ...
    /** Load cameras from a Meshrrom SFM cameras.sfm txt file.
		...
		* \note the camera frame is internally transformed to be consistent with fribr and RC.
		*/
    static std::vector<InputCamera::Ptr> loadMeshroom(const std::string &meshroomSFMPath, const float zNear = 0.01f,
                                                      const float zFar = 1000.0f);
