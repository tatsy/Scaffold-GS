...
void CameraRecorder::loadColmap(const std::string &filePath, int w, int h) {
    ...
    std::vector<InputCamera::Ptr> path =
        InputCamera::loadColmap(colmapDir.parent_path().string(), float(0.01), 1000, false);
...
void CameraRecorder::saveAsBundle(const std::string &filePath, const int height, const int step) {
    ...
    SIBR_LOG << "[CameraRecorder] - Saved " << _cameras.size() << " cameras to " << filePath << " (using fovy "
             << _cameras[0].fovy() << ")." << std::endl;
}
...
void CameraRecorder::saveAsColmap(const std::string &filePath, const int height, const int width) {
    ...
    for (int i = 0; i < _cameras.size(); i++) {
        ...
        sibr::Vector3f t =
            converter * _cameras[i].rotation().toRotationMatrix().transpose() * (-_cameras[i].position());

        out_images << i << " " << -_cameras[i].rotation().x() << " " << -_cameras[i].rotation().w() << " "
                   << -_cameras[i].rotation().z() << " " << _cameras[i].rotation().y() << " "
                   << _cameras[i].view()(0, 3) << " " << -_cameras[i].view()(1, 3) << " " << -_cameras[i].view()(2, 3)
                   << " " << i << " "
        ...
        out_cameras << i << " "
                    << "PINHOLE"
                    << " " << width << " " << height << " " << focal << " " << focal << " " << width / 2.0 << " "
                    << height / 2.0 << std::endl;
    }
