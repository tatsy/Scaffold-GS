...
SIBR_VIDEO_EXPORT sibr::Volume3u laplacianBlending(const sibr::Volume3u &vA, const sibr::Volume3u &vB,
...
void VideoUtils::getMeanVariance(cv::VideoCapture &cap, cv::Mat &outMean, cv::Mat &outVariance,
    ...
    bool doResize =
        (finalSize[0] != cap.get(cv::CAP_PROP_FRAME_WIDTH) || finalSize[1] != cap.get(cv::CAP_PROP_FRAME_HEIGHT));
    while (true) {
        std::cout << "." << std::flush;
...
void VideoUtils::getMeanVariance2(cv::VideoCapture &cap, cv::Mat &outMean, cv::Mat &outVariance,
                                  const sibr::Vector2i &finalSize, float starting_point_s) {
...
void VideoUtils::getBackGroundVideo(sibr::Video &vid, PyramidLayer &out_mask, PyramidLayer &out_video, cv::Mat &out_img,
                                    const sibr::ImageRGB &meanImg, int threshold, int numBins, float time_skip_begin,
                                    float time_skip_end) {
...
#pragma omp parallel for
    ...
    const std::string basename = folder + filepath.stem().string() + "_bila_" + std::to_string(radius_bila) + "_"
...
void VideoUtils::computeSaveVideoMaskBlur(Video &vid, int time_window) {
    ...
    const std::string in_filename = filepath.parent_path().string() + "/masks/bilateral/" + filepath.stem().string()
                                    + "_bila_21_100_median_binary.mp4";
...
cv::Mat VideoUtils::getTemporalSpatialRatio(sibr::Video &vid, PyramidLayer &out_ratio,
                                            const sibr::ImageRGB &spatial_ratio, int numBins, float time_skip_begin,
                                            float time_skip_end) {
    cv::Mat volume = vid.getVolume(time_skip_begin, time_skip_end).t();
            ...
            for (int t = 0; t < L; ++t) {
                ...
                for (int c = 0; c < 3; ++c) {
                    out_ratio.volume.at<uchar>(t, 3 * (i * w + j) + c) =
                        sibr::clamp<uchar>((uchar)(128 * ratios[c]), 0, 255);
...
void VideoUtils::simpleFlowSave(cv::VideoCapture &cap, float ratio, std::function<std::string(int flow_id)> naming_f) {
    ...
    simpleFlow(cap, ratio, [&](cv::Mat prev, cv::Mat next, cv::Mat flow, int flow_id) {
        ...
        return cv::imwrite(naming_f(flow_id).c_str(), viz);
    });
    std::cout << "done" << std::endl;
}
...
#pragma omp parallel for
    for (int t = 0; t < layer.l; ++t) {
        ...
        if (params.splacialDS) {
            cv::GaussianBlur(sliceIn, sliceOut, cv::Size(2 * params.spatial_radius + 1, 2 * params.spatial_radius + 1),
                             0);
        } else {
            ...
            for (int i = 0; i < layer.h; ++i) {
                for (int c = 0; c < 3; ++c) {
                    out.at<cv::Vec3b>(t, i)[c] = (uchar)sibr::clamp(
                        (int)layer.volume.at<float>(t, 3 * (i * layer.w + j) + c) + (center ? 128 : 0), 0, 255);
                }
            }
        }
    }
...
VideoGaussianPyramid buildVideoGaussianPyramid(const cv::Mat &volume, int w, int h, int nLevels,
                                               const PyramidParameters &params, bool show) {
...
VideoGaussianPyramid buildVideoGaussianPyramid(sibr::Video &vid, int nLevels, const PyramidParameters &params,
                                               bool show) {
    return buildVideoGaussianPyramid(vid.getVolume(), vid.getResolution()[0], vid.getResolution()[1], nLevels, params,
                                     show);
}

SIBR_VIDEO_EXPORT VideoLaplacianPyramid buildVideoLaplacianPyramid(PyramidLayer vid, int nLevels,
                                                                   const PyramidParameters &params, bool show) {
...
VideoLaplacianPyramid buildVideoLaplacianPyramid(sibr::Video &vid, int nLevels, const PyramidParameters &params,
                                                 bool show) {
...
VideoLaplacianPyramid buildVideoLaplacianPyramidFullyReduced(PyramidLayer vid, int nLevels,
                                                             const PyramidParameters &params, bool show) {
...
PyramidLayer videoLaplacianBlending(PyramidLayer vidA, PyramidLayer vidB, PyramidLayer mask_volume,
                                    PyramidParameters params, bool show) {
    ...
    auto pyrM =
        sibr::buildVideoGaussianPyramid(mask_volume.volume, mask_volume.w, mask_volume.h, num_lvls, params, show);
...
std::vector<FullContribData> videoLaplacianBlendingContrib(PyramidLayer vidA, PyramidLayer vidB,
                                                           PyramidLayer mask_volume, PyramidParameters params) {
    int num_lvls = params.num_levels;
...
void videoLaplacianBlendingDebug(PyramidLayer vidA, PyramidLayer vidB, PyramidLayer mask_volume,
