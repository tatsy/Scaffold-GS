...
void makeDirectory(const std::string &path) {
    fs::path p(path);
    if (fs::exists(p) == false) fs::create_directories(p);
}

std::vector<std::string> listFiles(const std::string &path, const bool listHidden, const bool includeSubdirectories,
            ...
            if (includeSubdirectories && fs::is_directory(dir_itr->status())) {
                ...
                if (shouldCheckExtension) {
                    for (const auto &allowedExtension : allowedExtensions) {
                        if (dir_itr->path().extension() == ("." + allowedExtension)
...
bool copyFile(const std::string &src, const std::string &dst, const bool overwrite) {
    ...
    try {
        ...
        if (!fs::exists(source) || !fs::is_regular_file(source)) {
            std::cerr << "Source file " << source.string() << " does not exist or is not a regular file." << '\n';
            return false;
        }
        ...
        if (fs::exists(destination) && !overwrite) {
            std::cerr << "Destination file " << destination.string() << " already exists." << '\n';
            return false;
        }
        if (overwrite) {
...
SIBR_SYSTEM_EXPORT std::string getInstallDirectory() {
    ...
    {
        SIBR_ERR << "Can't find install folder! Please specify as command-line option using --appPath option!"
                 << std::endl;
    }
...
SIBR_SYSTEM_EXPORT std::string getInstallSubDirectory(const std::string &subfolder) {
    ...
    if (!directoryExists(installSubDirectory)) {
        ...
        if (!directoryExists(installSubDirectory))
            SIBR_ERR << "Can't find subfolder " << subfolder << " in " << installDirectory
                     << ". Please specify correct app folder as command-line option using --appPath option!"
                     << std::endl;
    }
...
bool showFilePicker(std::string &selectedElement,
    ...
    if (mode == Directory) {
        ...
        result = NFD_SaveDialog(extensionsAllowed.empty() ? NULL : extensionsAllowed.c_str(), directoryPath.c_str(),
                                &outPath);
    } else {
        result = NFD_OpenDialog(extensionsAllowed.empty() ? NULL : extensionsAllowed.c_str(), directoryPath.c_str(),
                                &outPath);
    }
