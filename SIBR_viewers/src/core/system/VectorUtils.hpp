...
T_out sum(
    const std::vector<T_in> &vec, const std::function<bool(T_in)> &f = [](T_in val) { return true; }) {
    double sum = 0;
    for (T_in val : vec) {
        if (f(val)) {
            sum += (double)val;
...
/** Weighted sum of elements in a vector.
	 ...
	 **/
template <typename T_in, typename T_out = T_in>
std::vector<T_out> weighted_normalization(
    const std::vector<T_in> &vec, const std::vector<T_in> &weights,
...
/** Apply a function to each element of a vector (not in place)
	 ...
	 */
template <typename T_in, typename T_out = T_in>
std::vector<T_out> applyLambda(const std::vector<T_in> &vec, const std::function<T_out(T_in)> &f) {
...
std::vector<T_out> applyLambda(const std::vector<T_in> &vec1, const std::vector<T_in> &vec2,
...
/** Normalize all elements in a vector based on the min and max values in it (not in place).
	 ...
	 **/
template <typename T_in, typename T_out = T_in>
std::vector<T_out> normalizedMinMax(
    const std::vector<T_in> &vec, const std::function<bool(T_in)> &f = [](T_in val) { return true; }) {
    T_in min = 0, max = 0;
    bool first = true;
    for (T_in val : vec) {
...
/** Apply a power-sum normalization.
	 ...
	 *\return a vector containing the normalized values
	 */
template <typename T_in, typename T_out = T_in, unsigned int Power = 2>
std::vector<T_out> normalizedZeroOne(
    const std::vector<T_in> &vec, const std::function<bool(T_in)> &f = [](T_in val) { return true; }) {
    double sumP = 0;
