 ...
 */
SIBR_SYSTEM_EXPORT std::vector<std::string> listFiles(const std::string &path, const bool listHidden = false,
...
enum FilePickerMode { Default, Save, Directory };
 ...
 */
SIBR_SYSTEM_EXPORT bool showFilePicker(std::string &selectedElement, const FilePickerMode mode,
...
void taskTiming(const std::string &s, FunType &&f, ArgsType &&...args) {
    ...
    std::cout << s << " : " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms"
              << std::endl;
};
