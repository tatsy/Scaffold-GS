...
#define SIBR_WRG                                                                                      \
  std::cout << "[SIBR] !! WARNING !!:\tFILE " << __FILE__ << "\n\t\t\tLINE " << __LINE__ << ", FUNC " \
            << __FUNCTION_STR__ << "\n\t\t\t"
#define SIBR_ERR                                                                                               \
  ::sibr::LogExit() <<= std::cerr << "[SIBR] ##  ERROR  ##:\tFILE " << __FILE__ << "\n\t\t\tLINE " << __LINE__ \
                                  << ", FUNC " << __FUNCTION_STR__ << "\n\t\t\t"  // Could be augmented for exiting
...
// assert) so that we can retrieve the callstack easily for debugging).
  ...
  #define SIBR_ASSERT_LOGIC(condition) \
    (condition)  // This assertion can contain code logic (this code will also be included at release)
#else
  #define SIBR_ASSERT(condition)                               \
    do {                                                       \
      if (!(condition)) {                                      \
        SIBR_WRG << "ASSERT FAILED: " #condition << std::endl; \
        assert(condition);                                     \
      }                                                        \
    } while (0)
  #define SIBR_ASSERT_LOGIC(condition)                         \
...
#define SIBR_UNTESTED                                                               \
  SIBR_LOG << "!Warning! Using an untested code flagged as potentially "            \
              "unstable. (if something goes wrong, check over here - " __FILE__ ":" \
           << __LINE__ << ")" << std::endl;
#define SIBR_DEBUG(var)     \
  std::cout << __FILE__     \
      ":\n"                 \
      "[Debug] " #var " = " \
            << (var) << std::endl  // No access to debug mode for now (so I made this tmp tool)
...
#define SIBR_PROFILESCOPE                             \
  SIBR_PROFILESCOPE_EXPAND(                           \
      SIBR_CATMACRO(debugScopeProfiler, __COUNTER__), \
      std::string(__FUNCTION_STR__) + std::string(" (File: " __FILE__ ":" SIBR_MACROINTTOSTR(__LINE__) ")"));
#define SIBR_PROFILESCOPE_NAME(name) SIBR_PROFILESCOPE_EXPAND(SIBR_CATMACRO(debugScopeProfiler, __COUNTER__), name);
}  // namespace sibr
//# endif
