...
template <typename T>
static constexpr uint32_t NumberOfArg = 1;

/// uint contexpr helper, defining the number of command line tokens required to init T
template <>
static constexpr uint32_t NumberOfArg<bool> = 0;

/// uint contexpr helper, defining the number of command line tokens required to init T
template <>
static constexpr uint32_t NumberOfArg<Switch> = 0;

/// uint contexpr helper, defining the number of command line tokens required to init T
template <typename T, uint32_t N>
static constexpr uint32_t NumberOfArg<sibr::Vector<T, N>> = N *NumberOfArg<T>;
...
struct ValueGetter {
	 ...
	 */
    static T get(const std::vector<std::string> &values, uint32_t n);
...
enum RenderingModes { RENDERMODE_MONO, RENDERMODE_STEREO_ANAGLYPH, RENDERMODE_STEREO_QUADBUFFER };
...
protected:
    ...
    std::map<std::string, std::vector<std::string>>
        args;                                     ///< List of arguments input by the user and their parameters.
...
public:
    /** Constructor
		 ...
		 *\param description help message description
		 */
    RequiredArgBase(const std::string &_key, const std::string &description = "")
        : key(_key) {
...
struct SIBR_SYSTEM_EXPORT AppArgs {
    ...
    Arg<std::string> pathFile = {
        "pathFile", "", "filename of path to render offline; app renders path and exits"
    };  // app needs to handle this; if it does default behavior is to render the path and exit
    Arg<std::string> outPath = {
        "outPath", "pathOutput", "Path of directory to store path output default relative the input path directory "
    };  // app needs to handle this; if it does default behavior is to render the path and exit
};
...
/// \ingroup sibr_system
struct SIBR_SYSTEM_EXPORT WindowAppArgs : virtual AppArgs, virtual WindowArgs {};
...
/// \ingroup sibr_system
struct SIBR_SYSTEM_EXPORT BasicIBRAppArgs : virtual WindowAppArgs, virtual BasicDatasetArgs, virtual RenderingArgs {};
...
struct ValueGetter<Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>> {
    static Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> get(
        const std::vector<std::string> &values, uint n) {
